...
contract StakingPool is Ownable {
    ...
    function unstake() external whenNotPaused {
        ...
        require(
            block.timestamp >= stakeTimestamp[msg.sender] + (lockDays * 1 days),
            "Lock period not over"
        );

        uint256 durationDays = (block.timestamp - stakeTimestamp[msg.sender]) /
            1 days;
    ...
    function claimRewards() external whenNotPaused {
        ...
        require(
            stakeToken.balanceOf(address(this)) >= reward,
    ...
    function getPendingRewards(address user) external view returns (uint256) {
        ...
        uint256 durationDays = (block.timestamp - stakeTimestamp[user]) /
            1 days;
...
contract StakingFactory {
    ...
    uint256 public constant CREATE_FEE = 5000 * 10 ** 18;
